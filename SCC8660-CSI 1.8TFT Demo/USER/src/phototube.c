...
void UpDataPhotoTube()
{
  ...
  PhotoTubeValue[7]=gpio_get(C8);
  PhotoTubeValue[6]=gpio_get(C9);
  PhotoTubeValue[5]=gpio_get(C10);
  PhotoTubeValue[4]=gpio_get(C11);
  PhotoTubeValue[3]=gpio_get(C12);
  PhotoTubeValue[2]=gpio_get(C13);
  PhotoTubeValue[1]=gpio_get(C14);
  PhotoTubeValue[0]=gpio_get(C15);
  MidLineColInTube=0;
  for(i=0;i<8;i++)
